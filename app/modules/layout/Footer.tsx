...
interface FooterData {
  ...
  sections: Record<string, LinkItem[]>;
...
const Footer: React.FC = () => {
  const footerData: FooterData = {
    ...
    copyright: {
      years: `2025-${new Date().getFullYear()}`,
      company: "Hughes Sheet Metal™",
      href: "#"
    }
  };
  ...
  const renderLink = (item: LinkItem, index: number) => (
    <li key={index} className="mb-4">
      <a href={item.href} className="hover:underline transition-all duration-200">
        {item.label}
      </a>
    </li>
  );

  const renderSection = ([title, links]: [string, LinkItem[]], index: number) => (
  ...
  const renderSocialIcon = ([name, href]: [string, string], index: number) => (
    <li key={index}>
      <a
        href={href}
      ...
      >
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path fillRule="evenodd" d={socialIcons[name]} clipRule="evenodd" />
        </svg>
      </a>
    </li>
  );
      ...
      <div className="p-4 py-6 mx-auto max-w-screen-xl md:p-8 lg:p-10">
        <div className="grid grid-cols-2 gap-8 lg:grid-cols-6">
          ...
          <div className="col-span-2">
            <a href={footerData.brand.href} className="flex items-center mb-2 text-2xl font-semibold text-gray-900 sm:mb-0 dark:text-white">
              <Image
            ...
            <ul className="flex mt-5 space-x-6">
              {Object.entries(footerData.social).map(renderSocialIcon)}
            </ul>
          </div>

          {/* Footer Links Sections */}
          {Object.entries(footerData.sections).map(renderSection)}
        </div>
        ...
        <span className="block text-sm text-center text-gray-500 dark:text-gray-400">
          © {footerData.copyright.years}{' '}
          <a href={footerData.copyright.href} className="hover:underline transition-all duration-200">
